...
{
	encoder_pub_ = node_->create_publisher<rto_msgs::msg::EncoderReadings>("encoder_readings", 10);
	encoder_position_server_ = node_->create_service<rto_msgs::srv::SetEncoderPosition>("set_encoder_position", std::bind(&EncoderInputROS::setEncoderPositionCallback, this, std::placeholders::_1, std::placeholders::_2));
}
...
void EncoderInputROS::readingsChangedEvent(int velocity, int position, float current)
{
	...
	encoder_msg_.header.stamp = node_->now();
...
void EncoderInputROS::setEncoderPositionCallback(
	...
	[[maybe_unused]] std::shared_ptr<rto_msgs::srv::SetEncoderPosition::Response> res)
{
